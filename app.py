...
def profile(username):
    ...
    # the ["username"] at the end specifies that we only want
    ...
    if session["user"]:
        return render_template("profile.html.j2", username=username)
...
def add_task():
    ...
    categories = mongo.db.categories.find().sort(
        "category_name, 1")
    return render_template("add_task.html.j2", categories=categories)
...
def edit_task(task_id):
    ...
    # retrieve the task from the db by targeting it's ID
    task = mongo.db.tasks.find_one({"_id": ObjectId(task_id)})

    categories = mongo.db.categories.find().sort(
        "category_name, 1")
    # add the task variable to identify the task to be modified
    return render_template("edit_task.html.j2", task=task, categories=categories)
...
def add_category():
    ...
    return render_template("add_category.html.j2")
...
def edit_category(category_id):
    ...
    return render_template("edit_category.html.j2", category=category)
